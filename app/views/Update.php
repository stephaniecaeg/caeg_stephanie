<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Update</title>
     <style>
        /* update.css */

body {
    font-family: Arial, sans-serif;
    background: #ffe6e1; /* soft peach-pink background */
    margin: 0;
    height: 100vh;
    display: flex;
    flex-direction: column;
    align-items: center;
}

h1 {
    width: 100%;
    text-align: center;
    color: #cc3366; /* pig pink for header */
    margin: 20px 0;  /* smaller margin above and below */
    font-size: 1.8rem;
    font-weight: bold;
}

form {
    background: #fff5f2; /* light peach form background */
    padding: 30px 40px;
    border-radius: 15px;
    box-shadow: 0 4px 10px rgba(0,0,0,0.1);
    width: 400px;
    text-align: left;
    margin-top: 10px; /* slightly below header */
}

label {
    font-size: 0.95rem;
    font-weight: bold;
    color: #b03060;
    display: block;      /* block to sit above input */
    margin-bottom: 4px;  /* small space below label */
}

input[type="text"],
input[type="email"] {
    width: 80%;
    padding: 10px;
    margin: 0 0 15px 0;  /* no space above input, space below */
    border: 1px solid #f5b6b0;
    border-radius: 8px;
    outline: none;
    font-size: 1rem;
    background: #fff;
    transition: border 0.3s;
    display: block;
    margin-left: auto;
    margin-right: auto;  /* center input */
}

input[type="text"]:focus,
input[type="email"]:focus {
    border-color: #ff99aa; /* highlight on focus */
}

input[type="submit"] {
    width: 50%;
    padding: 12px;
    background: #ff99aa; /* pig pink */
    border: none;
    border-radius: 8px;
    color: white;
    font-size: 1rem;
    font-weight: bold;
    cursor: pointer;
    transition: background 0.3s;
    display: block;
    margin: 10px auto 0 auto; /* center horizontally with small top margin */
}

input[type="submit"]:hover {
    background: #e57a8a; /* darker peach hover */
}

     </style>
</head>
<body>
    <h1>Welcome to Update View</h1>
    <form action="<?=site_url('user/update/'.$student['ID']);?>" method="post">
        <label for="last_name">Last Name:</label><br>
        <input type="text" id="last_name" name="last_name" value="<?=html_escape($student['lastname']);?>"><br>
        
        <label for="first_name">First Name:</label><br>
        <input type="text" id="first_name" name="first_name" value="<?=html_escape($student['firstname']);?>"><br>
        
        <label for="email">Email:</label><br>
        <input type="email" id="email" name="email" value="<?=html_escape($student['email']);?>"><br>
        
        <label for="role">Role:</label><br>
        <input type="text" id="role" name="role" value="<?=html_escape($student['role']);?>"><br>
        
        <input type="submit" value="Submit">
    </form>

</body>
</html>